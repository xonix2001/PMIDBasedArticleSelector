...
class PubMedDataProcessorExtended(PubMedDataProcessorBase):
    def __init__(self, email, api_key, progress_signal=None, eta_signal=None):
        # 调用父类构造，只接受 email, api_key
        super().__init__(email, api_key)
        # 存储 UI 进度信号
        self.progress_signal = progress_signal
        self.eta_signal      = eta_signal
    ...
    def process_data(self, input_file_path):
        with open(input_file_path, 'r', encoding='utf-8') as f:
            pmid_list = [l.strip() for l in f if l.strip()]
        total = len(pmid_list)
        ...
        data = {k: [] for k in (
            "PMID", "Title", "Title_CN", "Year", "Journal",
        ...
        with ThreadPoolExecutor(max_workers=20) as executor:
            ...
            for future in as_completed(future_to_pmid):
                ...
                eta_sec = avg_per * remain

                now = datetime.now().strftime("%H:%M:%S")
                print(f"[{now}] [{completed}/{total}] 已处理 PMID {pmid} "
                      f"| 已用 {elapsed:.1f}s | 预计剩 {int(eta_sec)}s")

                # 发射 UI 信号
                if self.progress_signal:
                    self.progress_signal.emit(completed, total)
                if self.eta_signal:
                    self.eta_signal.emit(eta_sec)

        # 构建 DataFrame、保存 Excel、格式化、加超链接…（同原逻辑）
    ...
    def fetch_and_process_pmid(self, pmid):
        ...
        try:
            ...
            return {
                "PMID": pmid, "Title": title, "Title_CN": title_cn,
                "Year": pub_date, "Journal": journal,
                "Crossref-Cites": cites, "Abstract": abstract,
                "Abstract_CN": abstract_cn, "URL": url
            }
        except Exception as e:
